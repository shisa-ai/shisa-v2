...
def main():
    # --- Configuration ---
    final_output_file = "dpo.shisa-v2.1o.jsonl"
    random_seed = 42
    ...
    try:
        ds_rp = load_dataset("shisa-ai/shisa-v2-roleplaying-dpo", split="train[:10%]")
    ...
    try:
        ds_tl = load_dataset("shisa-ai/translation-no-extra-text-dpo-dataset", split="train[:10%]")
    ...
    try:
        ds_if = load_dataset("shisa-ai/instruction-following-dpo-combined-20251010", split="train[:10%]")
    ...
    try:
        ds_kiseki = load_dataset("shisa-ai/chotto_kiseki_dpo_9_19_25", split="train[:10%]")
