...
INIT_CHECKPOINT="${INIT_CHECKPOINT:-}"  # optional Megatron checkpoint to warm-start from
...
if [[ ! -f "${VOCAB_FILE}" || ! -f "${MERGE_FILE}" ]]; then
    echo "ERROR: Expected tokenizer files ${VOCAB_FILE} and ${MERGE_FILE} to exist. Run 02-generate.sh first." >&2
    exit 1
fi
