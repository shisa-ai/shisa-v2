...
"""
TRL + MegaBlocks training script for Qwen3-30B-A3B-v2 SFT on the dedicated 8x MI300X box.
Converted from the old bash launcher so we can keep all knobs in one place and layer in the kernels integration.
"""
...
def register_megablocks_kernel() -> bool:
    ...
    try:
        ...
        use_kernel_forward_from_hub("Qwen2MoeSparseMoeBlock")(Qwen2MoeSparseMoeBlock)
    except Exception as exc:
        log(f"Failed to prepare Qwen2 block for kernels: {exc}")
        return False
