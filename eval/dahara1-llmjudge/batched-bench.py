...
class VLLMBackend(InferenceBackend):
    ...
    def __init__(self, model_name: str, batch_size: int, gpus: int):
        from vllm import LLM, SamplingParams
        from vllm.engine.async_llm_engine import AsyncLLMEngine
        self.model_name = model_name
        self.batch_size = batch_size

        # Based on 8 GPUs we calculate the tp/pp
        '''
        TODO:
        tp=gpus
        pp=int(8/gpus)
        pipeline_parallel_size=pp,
        requires AsyncLLMEngine
        https://docs.vllm.ai/en/latest/dev/engine/async_llm_engine.html
        https://www.kaggle.com/code/bobfromjapan/vllm-asyncllmengine-demonstration?scriptVersionId=187070632&cellId=4
        '''
...
def create_inference_backend(model_name: str, batch_size: int, gpus: int) -> InferenceBackend:
    ...
    try:
        ...
        try:
            backend = VLLMBackend(model_name, batch_size, gpus)
    ...
    # You know this path might not work right now...
...
class ProcessingStats:
    ...
    model_name: str
    backend: str
    model_load_time: float  # Added model load time
...
def load_jsonl_batch(input_file: str, start_idx: int, batch_size: int) -> (List[Dict], bool):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        if idx + 1 >= sum(1 for _ in open(input_file, 'r', encoding='utf-8')):
            is_end = True
...
def process_model(
    ...
    try:
        ...
        backend = create_inference_backend(model_name, batch_size, gpus)
        if not backend:
            return
        ...
        total_examples_in_file = sum(1 for _ in open(input_file, 'r', encoding='utf-8'))
        pbar = tqdm(total=total_examples_in_file, desc=f"Processing {model_name}", unit="examples", initial=current_idx, dynamic_ncols=True)
        ...
        processing_time = total_time - model_load_time

        # Calculate final statistics
        processing_stats = ProcessingStats(
            total_examples=len(processed_examples),
            processing_time=processing_time,
            examples_per_second=len(processed_examples) / processing_time if processing_time > 0 else 0,
            total_tokens=stats['total_tokens'],
            tokens_per_second=stats['total_tokens'] / processing_time if processing_time > 0 else 0,
...
def main():
    ...
    flash_attn = os.getenv('VLLM_USE_TRITON_FLASH_ATTN')
    print(f"VLLM_USE_TRITON_FLASH_ATTN is set to: {flash_attn}")
    BATCH_SIZE = 2048  # Adjust based on your GPU memory
    print(f"Batch size is set to: {BATCH_SIZE}")

    all_models = [
        ["meta-llama/Llama-3.1-8B-Instruct", 1],
        ["alpindale/WizardLM-2-8x22B", 1],
        ["CohereForAI/c4ai-command-r-plus-08-2024", 1],
        ["deepseek-ai/DeepSeek-V2.5", 2],
        ["meta-llama/Llama-3.1-405B-Instruct", 4],
        ["meta-llama/Llama-3.1-70B-Instruct", 1],
        ["nvidia/Llama-3.1-Nemotron-70B-Instruct-HF", 1],
    ...
    # BUG/HACK !!! vllm.LLM does not like multiple models, sigh
    args = parse_arguments()
    selected_model_name = args.model_name

    # Find the GPU count for the selected model
    model_entry = next(
        (model for model in all_models if model[0] == selected_model_name), None
    )
    if not model_entry:
        gpus = 8
        print(f"Error: Model '{selected_model_name}' not found in the model list.")
        return

    gpus = model_entry[1]

    # Inject the single model into all_models list
    all_models = [[selected_model_name, gpus]]


    results = []
    for model_name, gpus in all_models:
        ...
        if stats:
            ...
            # Print summary after each model with extra line breaks
            logging.info("\n" + "-" * 50)
            logging.info(f"Results for {model_name}:")
            logging.info(f"Backend: {stats.backend}")
            logging.info(f"Model load time: {stats.model_load_time:.2f} seconds")
            logging.info(f"Processing time: {stats.processing_time:.2f} seconds")
            logging.info(f"Total time: {stats.model_load_time + stats.processing_time:.2f} seconds")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Process a single model for inference.")
    parser.add_argument(
        "model_name",
        type=str,
        help="The name of the model to process (e.g., 'Qwen/Qwen2.5-32B-Instruct')",
    )
